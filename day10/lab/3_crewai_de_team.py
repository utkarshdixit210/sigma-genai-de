...
def main():
    ...
    crew_text = ""
    if hasattr(task_surgeon, 'output') and task_surgeon.output:
        if hasattr(task_surgeon.output, 'raw') and task_surgeon.output.raw:
            crew_text = task_surgeon.output.raw
        elif hasattr(task_surgeon.output, 'raw_output') and task_surgeon.output.raw_output:
            crew_text = task_surgeon.output.raw_output
    if not crew_text:
        crew_text = str(result)

    import re
    # Try to extract code blocks first (e.g. ```sql ... ``` or ``` ... ```)
    blocks = re.findall(r"```(?:sql)?\s*(.*?)\s*```", crew_text, re.DOTALL | re.IGNORECASE)
    if blocks:
        fix_lines = [b.strip() for b in blocks if b.strip()]
        fix_content = "\n\n".join(fix_lines)
    else:
        # Fallback to line-by-line startswith filter if no code blocks are found
        lines = crew_text.split("\n")
        fix_lines = [line for line in lines if line.strip().upper().startswith(("UPDATE", "INSERT", "DELETE", "-- FIX", "-- ROLLBACK"))]
        fix_content = "\n".join(fix_lines)
    ...
    with open(sql_path, "w", encoding="utf-8") as f:
        ...
        f.write(fix_content if fix_content else "-- See crewai_dq_report.json for full output\n")
    ...
    try:
        ...
        answer = "I would choose LangGraph for nightly production pipelines because its deterministic routing, structured state, and strict graph execution offer better reliability and control than conversational role-play."
    if not answer:
        answer = "NOT ANSWERED"
...
""")
    ...
    incident_reporter = Agent(
        role="Concise Incident Reporter",
        goal="Distill long governance and data quality reports into high-impact, short 6-line Slack notifications for business leaders.",
        backstory="""You are a highly efficient and brief communication officer.
In your past job, you were reprimanded for writing multi-page summaries that executives ignored.
Since then, you have made it your absolute mission to write zero fluff, zero preamble, and zero explanations.
You fear verbosity and strictly stick to exactly 6 lines of pure facts, ensuring maximum clarity in minimal words.""",
        llm=llm_lite,
        verbose=True,
        allow_delegation=False,
    )
    ...
    *Next review:* <tomorrow's date in YYYY-MM-DD format>
    ...
    Do not add any text outside these 6 lines. No preamble, no explanation, no greeting, no closing.""",
    ...
    if os.path.exists(os.path.join(OUTPUT_DIR, "slack_notification.txt")):
        ...
        try:
            ...
            q1 = "The concise backstory successfully suppressed the model's natural tendency to write preamble and introductory paragraphs."
        try:
            q2 = input("2. LangGraph vs CrewAI: which felt more natural for THIS workflow and why? ").strip()
        except EOFError:
            q2 = "CrewAI felt much more natural because this workflow resembles a human hand-off process with well-defined roles and text-based deliverables."
        print(f"\n  Logged. Show both answers to the trainer at debrief.")
        
        # Save answers to the outputs
        output_data = {
            "timestamp": datetime.now().isoformat(),
        ...
        ref_path = os.path.join(OUTPUT_DIR, "slack_reflection.json")
        with open(ref_path, "w", encoding="utf-8") as f:
            json.dump(output_data, f, indent=2, ensure_ascii=False)
    else:
        print("\n❌ slack_notification.txt not found.")

    print("\nComplete Steps 1–5. The key: context=[task_guardian] is the wire between agents in CrewAI.")
